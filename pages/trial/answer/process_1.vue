...
export default {
  ...
  methods: {
    ...
    ban(id, index) {
      this.$axos.$post('admin/trial/answer/ban', { id }).then(() => {
        this.list.splice(index, 1)
        this.$toast.success('操作成功')
      })
    },
    pass(id, index) {
      this.$axios.$post('admin/trial/answer/pass', { id }).then(() => {
        this.list.splice(index, 1)
        this.$toast.success('操作成功')
      })
    },
    approve(id, index) {
      this.$axios.$post('admin/trial/answer/approve', { id }).then(() => {
        this.list.splice(index, 1)
        this.$toast.success('操作成功')
      })
    },
    reject(id, index) {
      this.$axios.$post('admin/trial/answer/reject', { id }).then(() => {
        this.list.splice(index, 1)
        this.$toast.success('操作成功')
      })
    }
